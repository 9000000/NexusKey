...
inline constexpr SettingMeta kSettings[] = {
    ...
    NK_TYPING("auto-caps-raw-macro",  autoCapsRawMacro,
              "Giữ phím gốc cho gõ tắt","Preserve raw keys for macros",
              L"Giữ phím bấm thực tế khi tự động viết hoa đầu câu để không làm lệch phím tắt (ví dụ: gõ nMa vẫn khớp phím tắt nMa dù chữ n đổi thành N)",
              L"Preserve physical raw keys during sentence auto-capitalization so custom macro shortcuts remain matched (e.g. typing nMa matches nMa even when n becomes N)", 2221, 0, 0),
    ...
    NK_ACTION("btn-reset-floating-icon",
              "Đặt lại",              "Reset",
              L"Đặt lại vị trí của icon nổi về góc mặc định",
              L"Reset the floating icon position to default corner",    2508, 2, 1),
