...
### §3 — IPC + Concurrency (SPSC ring buffer + self-healing) &nbsp;&nbsp;`[STATUS: watchdog shipped PR #154; HookSelfHealer reverted 2026-05-17; SPSC ring closed 2026-05-09 — see plans/2026-05-09-hook-engine-ring-buffer-kill.md]`
...
- ~~**Minimal hook + watchdog**~~ — **PARTIAL via PR #154** (`d37d0e1`, 2026-05-08). `HeartbeatPublisher` (30s pulse) + `VKeyWatchdog.exe` (sidecar process) **shipped and active**. `HookSelfHealer` (Raw Input dual-channel hijack detector) **REVERTED 2026-05-17**: `RIDEV_INPUTSINK` registration silently routed own-process keystrokes through Raw Input, bypassing `WH_KEYBOARD_LL` and breaking VN typing in every Classic dialog textbox. Anti-Dorion coverage continues through `WM_APP_REINSTALL_HOOKS` (proactive reinstall on focus → Chromium / Electron / Java). Reactive mid-session hijack detection is no longer covered; revisit only if field reports surface a real hijack vector outside the proactive set.
...
| §3 SPSC + Watchdog | Watchdog shipped (PR #154); `HookSelfHealer` reverted 2026-05-17; SPSC ring closed | 2026-05-08 / 2026-05-09 / 2026-05-17 | Watchdog: `HeartbeatPublisher` + `VKeyWatchdog.exe` (active). `HookSelfHealer` reverted — `RIDEV_INPUTSINK` bypassed LL hook for own-process, breaking Classic dialog VN typing; Anti-Dorion coverage via `WM_APP_REINSTALL_HOOKS`. SPSC ring closed per `docs/plans/2026-05-09-hook-engine-ring-buffer-kill.md` — measured baselines + sync-vs-async UX trade-off do not justify the rebuild. |
