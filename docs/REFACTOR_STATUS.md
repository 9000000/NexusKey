...
| ~~H6a~~ | ~~**Watchdog / self-healing**~~ — `HeartbeatPublisher` + `VKeyWatchdog.exe` (active). `HookSelfHealer` reverted 2026-05-17: `RIDEV_INPUTSINK` bypassed `WH_KEYBOARD_LL` for own-process → Classic dialog VN typing broken. Anti-Dorion now via `WM_APP_REINSTALL_HOOKS` only. | CODE_GOVERNANCE.md §3 | DONE (partial) | ✅ PR #154 `d37d0e1`; partial revert 2026-05-17 |
